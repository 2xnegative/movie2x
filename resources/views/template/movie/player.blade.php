...
@if($movie->type == "movie" || $movie->type == "av")
    <div class="player_container" id="player_movie" style="display: {{ $ads_movie_video_count == 0 ? 'block' : 'none' }};">
        <div style="display: block; padding: 15px;" class="movie_player">
            ...
            @endphp
            <iframe src="{{ $file_main }}" id="player_iframe" style="width: 100%; height: 420px; border: none;/* position: relative; z-index: 2147483647*/"
...
@elseif($movie->type == "series" || $movie->type == "anime")
    ...
    <div class="player_container" id="player_movie" style="display: {{ $ads_movie_video_count == 0 ? 'block' : 'none' }};">
        <div style="display: block; padding: 15px;" class="movie_player">
            ...
            @endphp
            <iframe src="{{ $filemovie[1][0] != "" ? $filemovie[1][0] : '' }}" id="player_iframe" style="width: 100%; height: 420px; border: none;/* position: relative; z-index: 2147483647*/"
    ...
    @endphp
        @for ($i=0; $i < $count_filename; $i++)
            ...
            <div class="episode" style="padding: 0 20px;color: #fff;text-align:center;">
                @if($i == 0)
                    <div class="episode_path"  data-ep-name="{{ $movie->title }} - {{ $filename[1][$i] }}" data-href="{{ $source }}" style="width: 710px; background-color: #127ba3;cursor: pointer;padding: 10px;border-radius: 5px;margin: 5px">
                        <i class="fas fa-play"></i> {{ $movie->title }} - {{ $filename[1][$i] }}
                    </div>
                @else
                    <div class="episode_path"  data-ep-name="{{ $movie->title }} - {{ $filename[1][$i] }}" data-href="{{ $source }}" style="width: 710px; background-color: #5b5b5b;cursor: pointer;padding: 10px;border-radius: 5px;margin: 5px">
                        <i class="fas fa-play"></i> {{ $movie->title }} - {{ $filename[1][$i] }}
                    </div>
                @endif
            </div>
